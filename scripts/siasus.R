...
### montar as bases de dados do SIASUS/DataSUS.
### A idéia é construir um catálogo com a função catalog_siasus(),
### filtrar (ou não) os dados desejados, depois montar as bases de dados
### no disco usando a função build_siasus().
...
catalog_siasus <- function( output_dir , drop.prelim = TRUE ){
  
  # define pastas-raízes
  sus_htmls <- c( "ftp://ftp.datasus.gov.br/dissemin/publicos/SIASUS/200801_/" , "ftp://ftp.datasus.gov.br/dissemin/publicos/SIASUS/199407_200712/Dados/" )
  ...
  catalog <-
    data.frame(
      ...
      type = paste0( "siasus_" , gsub( ".*SIASUS\\/|\\/.*|_" , "" , these_links ) ) ,
      stringsAsFactors = FALSE
    )
  
  # define subtipos a partir de 2008
  these_subtype_codes <- toupper( basename( catalog[ catalog$type == "siasus_200801" , "full_url" ] ) )
  these_subtype_codes <- toupper( gsub( "[A-Z]{2}[0-9]{4}[A-Z]{0,1}\\..*" , "" , these_subtype_codes ) )
  catalog[ catalog$type == "siasus_200801" , "subtype_code" ] <- these_subtype_codes
  
  # de 1994-2007, só existia o código "pa"
  catalog[ catalog$type == "siasus_199407200712" , "subtype_code" ] <- "PA"
  
  # cria df com descrição de subtipos
  stype_df <- data.frame( subtype_code = c("AB", "ABO", "ACF", "AD", "AM", "AMP", "AN", "AQ", "AR", "ATD", "BI", "PA", "PS", "SAD") , 
                          subtype = c( "bariatrica", "pos_bariatrica", "fistula_arteriovenosa", "laudos_diversos", "medicamentos", 
                                       "multiprofissional", "nefrologia", "quimioterapia", "radioterapia", "dialitico", "bpai", 
                                       "producao_ambulatorial", "psicossocial", "atencao_domiciliar" ) , stringsAsFactors = FALSE )
  
  # descarta subtipos sem descrição
  stype_df <- stype_df[ !is.na( stype_df$subtype ) , ]
  
  # combina com catalogo
  catalog <- merge( catalog , stype_df , by = "subtype_code" , all.x = TRUE )
  
  # define mês e ano
  these_timestamps  <- gsub( "[^0-9]" , "" , basename( catalog[ , "full_url" ] ) )
  these_years <- as.numeric( substr( these_timestamps , 1 , 2 ) )
  these_years <- ifelse( these_years > 90 , 1900 + these_years , 2000 + these_years )
  these_months <- as.numeric( substr( these_timestamps , 3 , 4 ) )
  catalog[ , "year" ] <- these_years
  catalog[ , "month" ] <- these_months
  
  # coleta ufs
  these_ufs  <- gsub( "[0-9].*" , "" , toupper( basename( catalog[ , "full_url" ] ) ) )
  these_ufs <- ifelse( nchar( these_ufs ) > 4 , substr( these_ufs , 4 , 5 ) , substr( these_ufs , 3 , 4 ) )
  catalog[ , "uf" ] <- these_ufs 
  ...
  catalog[ , "output_filename" ] <- 
    file.path( output_dir , 
               catalog[ , "type" ] , 
               catalog[ , "subtype" ] , 
               gsub( "\\.db(c|f)$" , ".fst" , tolower( basename( catalog[ , "full_url" ] ) ) ) )
  
  # reordena catálogo
  catalog <- catalog[ order( catalog[ , "subtype" ] , catalog[ , "year" ] , catalog[ , "uf" ] ) , ]
...
# cria datavault
datavault_siasus <- function( catalog , datavault_dir , skipExist = TRUE ) {
  ...
  # mostra mensagem
  cat( "\nsiasus datavault was built at" , datavault_dir , "\n" )
...
build_siasus <- function( catalog , skipExist = TRUE ) {
  ...
  for ( i in seq_len( nrow( catalog ) ) ){
    ...
    # acompanhamento de processo
    cat( paste0( "siasus catalog entry " , i , " of " , nrow( catalog ) , " stored at '" , catalog[ i , 'output_filename' ] , "'\r" ) )
...
monetdb_siasus <- function( catalog , skipExist = TRUE ) {
  ...
  # acompanhamento de processo
  cat( "\nsiasus monetdb process started.\n" )
  ...
  # acompanhamento de processo
  cat( "\nsiasus monetdb database was built." )
