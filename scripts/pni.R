...
### montar as bases de dados do PNI/DataSUS.
### A idéia é construir um catálogo com a função catalog_pni(),
### filtrar (ou não) os dados desejados, depois montar as bases de dados
### no disco usando a função build_pni().
...
catalog_pni <- function( output_dir , drop.prelim = TRUE ){
  
  # define pastas-raízes
  sus_htmls <- "ftp://ftp.datasus.gov.br/dissemin/publicos/PNI/"
  ...
  these_links <- these_links[ !grepl( ".*BR[0-9]{2}" , basename( these_links ) , ignore.case = FALSE ) ]
  
  # mantém apenas arquivos de dados
  these_links <- these_links[ grepl( "dados.*\\.(dbc|dbf)$" , these_links , ignore.case = TRUE ) ]
  ...
  catalog <-
    data.frame(
      ...
      type = "pni" ,
      subtype = ifelse( grepl( "^DPNI" , basename(these_links) ) , "doses" , "cobertura" ) ,
      stringsAsFactors = FALSE
    )
  
  # define nome do arquivo destino
  catalog$output_filename <-
    gsub( "\\.db(c|f)$" , ".fst" , file.path( output_dir , catalog$type , catalog$subtype , tolower( basename( these_links ) ) ) , ignore.case = TRUE )
...
# cria datavault
datavault_pni <- function( catalog , datavault_dir , skipExist = TRUE ) {
  ...
  # mostra mensagem
  cat( "\npni datavault was built at" , datavault_dir , "\n" )
...
build_pni <- function( catalog , skipExist = TRUE ) {
  ...
  for ( i in seq_len( nrow( catalog ) ) ){
    ...
    # acompanhamento de processo
    cat( paste0( "pni catalog entry " , i , " of " , nrow( catalog ) , " stored at '" , catalog[ i , 'output_filename' ] , "'\r" ) )
...
monetdb_pni <- function( catalog , skipExist = TRUE ) {
  ...
  for ( i in seq_len( nrow(dbentries) ) ) {
    ...
    if ( dbExistsTable( db , dbentries[ i , "db_tablename" ] ) ) {
      ...
      if ( skipExist ) { 
        ...
        next() 
      } else { dbRemoveTable( db , dbentries[ i , "db_tablename" ] ) }
    ...
    if( ncol( data_structure ) > 2 ) {
      data_structure$col_format <- apply( data_structure[ , -1 ] , 1 , function( y ) { ifelse( any( y %in% c( "character", "date" ) ) , "character" ,"numeric" ) } )
    } else {
      data_structure$col_format <- ifelse( any( data_structure[ , -1 ] %in% c( "character", "date" ) ) , "character" ,"numeric" ) 
    }
    ...
    for ( this_file in these_files ) {
      ...
      if (length(these_num_cols) > 0) x[ , (these_num_cols) := lapply( .SD , as.numeric ) , .SDcols = these_num_cols ]
      these_char_cols <- data_structure$column_name[ data_structure$col_format == "character" ]
      if (length(these_char_cols) > 0) x[ , (these_char_cols) := lapply( .SD , as.character ) , .SDcols = these_char_cols ]
  ...
  # acompanhamento de processo
  cat( "\npni monetdb database was built." )
