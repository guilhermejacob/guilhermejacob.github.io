...
# datavault
datavault_censo_escolar <- function( catalog , datavault_dir , skipExist = TRUE ) {
  ...
  # print message
  cat( "\ncenso_escolar datavault was built at" , datavault_dir , "\n" )
  ...
  function( catalog ){
    ...
    for ( i in seq_len( nrow( catalog ) ) ){
      ...
      if ( catalog[ i , "year" ] == 2017 ) {
        unzipped_files_2 <- sapply( unzipped_files , function ( x ) URLdecode( x ) , USE.NAMES = FALSE )
        unzipped_files_2 <- sapply( unzipped_files_2 , function ( x ) iconv( x , from = "850" ) , USE.NAMES = FALSE )
        
        for ( j in seq_along( unique( dirname( unzipped_files ) ) ) ) {
          
          if ( ( unique( dirname( unzipped_files_2 ) ) %in% unique( dirname( unzipped_files_2 ) ) )[ j ] ) {
            if ( !dir.exists( unique( dirname( unzipped_files_2 ) ) [ j ] ) ) { 
              dir.create( unique( dirname( unzipped_files_2 ) ) [ j ] ) } 
          }
        }
        
        file.rename( unzipped_files , unzipped_files_2 )
        
        for ( j in seq_along( unique( dirname( unzipped_files ) ) ) ) {
          if ( ( unique( dirname( unzipped_files_2 ) ) %in% unique( dirname( unzipped_files_2 ) ) )[ j ] ) {
            suppressWarnings( unlink( dirname( unzipped_files )[ j ] , recursive = TRUE , force = TRUE ) )
          }
        }
        
        unzipped_files <- unzipped_files_2
        rm( unzipped_files_2 )
      ...
      if( catalog[ i , 'year' ] <= 2006 ){
        ...
        for ( zipfile in grep( "\\.zip$" , unzipped_files , ignore.case = TRUE , value = TRUE ) ) { unzipped_files <- unique( c( unzipped_files , custom_extract( zipfile = zipfile , ext_dir = lodown:::np_dirname( zipfile ) , rm.main = TRUE ) ) ) }
        ...
        for ( j in seq( nrow( these_tables ) ) ){
          ...
          x <- lodown:::read_SAScii( these_tables[ j , 'data_file' ] , these_tables[ j , 'sas_script' ] , na_values = c( "" , "." ) )
          ...
          cat( tolower( gsub( "\\..*" , "" , basename( these_tables[ j , 'data_file' ] ) ) ) , "stored at" , these_tables[ j , 'db_tablename' ] , "\r\n\n" )
        ...
        for ( this_table_type in c( "docente" , "matricula" , "turma" , "escola" ) ) {
          ...
          for ( j in seq_along( tabelas ) ) {
            ...
            if ( grepl( "\\.rar$" , tabelas[ j ] , ignore.case = TRUE ) ) {
              ...
              this_data_file <- custom_extract( normalizePath( tabelas[ j ] ) , ext_dir = lodown:::np_dirname( normalizePath( tabelas[ j ] ) ) , rm.main = TRUE )
        ...
        this_excel_file <- unzipped_files [ grep( "^Dicionário|Dicionario" , basename( unzipped_files ), value = FALSE, ignore.case = TRUE ) ]
        data_files <- grep( "\\.rar$|\\.csv|\\.zip$", unzipped_files, value = TRUE , ignore.case = TRUE )
        ...
        for ( this_table_type in c( "docente" , "matricula" , "turma" , "escola" ) ) {
          ...
          for ( j in seq_along( tabelas ) ) {
            ...
            if ( grepl( "\\.(rar|zip)$" , tabelas[ j ] , ignore.case = TRUE ) ) {
              ...
              suppressMessages(
                tryCatch(
                  DBI::dbWriteTable(
                    ...
                    lower.case.names = TRUE ,
                    overwrite = TRUE ,
                    temporary = ifelse( length( tabelas ) > 1 , TRUE , FALSE ) ,
                    colClasses = ifelse( grepl( "num" , codebook$Tipo , ignore.case = TRUE ) , "numeric" , "character" )
                  ) , error = function(e) {
                    cat( "removing non-utf8 characters\r" )
                    cleaned_data_file <- remove_nonutf8_censo_escolar( this_data_file ) ;
      ...
      cat( paste0( "censo_escolar" , " catalog entry " , i , " of " , nrow( catalog ) , " stored in '" , catalog[ i , 'dbfolder' ] , "'\r\n\n" ) )
      
    }
    
    # disconnect from the current monetdb database
