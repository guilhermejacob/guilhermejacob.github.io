...
tags: [ "rstats" , "DataViz" , "Brazil" , "Educação" , "Desigualdade" ]
...
```{r coleta_dados, message=FALSE, warning=FALSE, cache=FALSE, include=FALSE, eval=FALSE}
# carrega libraries
library(DBI)
library(MonetDBLite)
library(here)
...
# salva imagem
save.image( file = here( "resources/enem2015.RData" ) )
```

Em outras palavras: estamos trabalhando com a *interseção* destes quatro conjuntos de examinados, *não a união*.

```{r carrega_resultados, echo=FALSE, message=FALSE, warning=FALSE , cache = FALSE } 
# carrega libraries
library(data.table)
library(here)
library(ggplot2)
...
# carrega dados
load( here( "resources/enem2015.RData" ) )

# transforma em data.table
lower_part <- data.table( lower_part ) 
upper_part <- data.table( upper_part )
...
# empilha dados
enem_df <- rbindlist( list( upper_part , lower_part ) )
rm( list = c( "upper_part" , "lower_part" ) ) ; gc()
```
...
these_cols <- grep( "^no_" , colnames( enem_df ) , value = TRUE )
enem_df[ , c( these_cols ) := lapply( .SD , function(z) gsub("[[:punct:]]", "", z ) ) , .SDcols = these_cols ]

# densidade
dist_extreme <- ggplot( enem_df , aes( x = q006 , group = marcador , fill = marcador )) +
...
# calcula médias
media_desempenho <- enem_df[ , .( media = mean( desempenho ) ) , by = .( marcador ) ]
...
```{r estima_desigualdade, echo=FALSE, message=FALSE, warning=FALSE , cache = FALSE , include = FALSE , eval=FALSE }
# carrega libraries
library(DBI)
library(MonetDBLite)
library(survey)
...
# salva imagem
save.image( file = here( "resources/enem2015_desigualdade.RData" ) )
```
```{r tabela1, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE }
# carrega libraries
library(here)
library(survey)
library(convey)
...
# carrega dados
load( here( "resources/enem2015_desigualdade.RData" ) )
...
```{r tabela2, echo=FALSE, message=FALSE, warning=FALSE, cache=FALSE }
