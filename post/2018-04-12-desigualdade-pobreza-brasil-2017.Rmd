...
```{r database_setup_2016, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}
# define diretórios
...
mid_year_price <- lapply( split( mid_year_price , f = list( mid_year_price$uf ) ) , function (x) { x[ , "ratio" ] <- x[ , "deflt" ] / x[ x$ano == 2017 , "deflt" ] ; x[ , c(1:2,4) ] })
mid_year_price <- do.call( rbind , mid_year_price )
...
# add age column
pnadc_df$age <- ifelse( !is.na( pnadc_df$v20082 ) , 2016 - pnadc_df$v20082 , NA )
...
# save results
est2016 <- list( res1 , res2 , res4 , res5 , res6 )

# clean workspace
rm( list = grep( "^est[0-9].*" , ls() , invert = TRUE , value = TRUE ) )
```
```{r database_setup_2017, message=FALSE, warning=FALSE, cache=TRUE, include=FALSE}
...
these_designs <- these_designs[ grepl( "2017 1" , these_designs ) ]

this_file <- these_designs[[1]]

# read dataset
these_columns <- c("upa", "estrato", "v1031", "v1030", "posest", "ano", "trimestre", "uf", "capital", "v2010", "v2009", "vd4016", "vd5001", "vd5002", "vd5003", "vd5004", "vd5005", "vd5006", "v3002", "vd4020", "vd2002", "v20082")
pnadc_df <- read_fst( this_file , columns = these_columns )
...
# add age column
pnadc_df$age <- ifelse( !is.na( pnadc_df$v20082 ) , 2017 - pnadc_df$v20082 , NA )
...
# save results
est2017 <- list( res1 , res2 , res4 , res5 , res6 )

# clean workspace
rm( list = grep( "^est[0-9].*|pnadc_design_chld_abrinq" , ls() , invert = TRUE , value = TRUE ) ) ; gc()
```
...
E, a propósito, podemos ver que os números de 2017^[Os números de 2016 do nosso post são um pouco diferentes, já que, ao contrário da publicação do IBGE, ajustamos as rendas para preços médios de 2017.] coincidem com [esta publicação do IBGE](https://biblioteca.ibge.gov.br/visualizacao/livros/liv101559_informativo.pdf):
