...
# LOESS
dt_saldo[ , c("total_season" , "total_trend" , "total_remainder") := data.frame( stl( ts( total , frequency = 12 ) , robust = TRUE , s.window = 11 , s.degree = 1 )$time.series ), by = .( tipo_mov ) ]
dt_saldo[ , total_d := total - total_season ]

dt_saldo_sec <- subset( dt_saldo_sec , !is.na( cnae_2_0_secao ) )
dt_saldo_sec[ , c("total_season" , "total_trend" , "total_remainder") := data.frame( stl( ts( total , frequency = 12 ) , robust = TRUE , s.window = 11 , s.degree = 1 )$time.series ), by = .( tipo_mov , cnae_2_0_secao ) ]
dt_saldo_sec[ , total_d := total - total_season ]

dt_saldo_uf <- subset( dt_saldo_uf , !is.na( uf ) )
dt_saldo_uf <- subset( dt_saldo_uf , !is.na( cnae_2_0_secao ) )
dt_saldo_uf[ , c("total_season" , "total_trend" , "total_remainder") := data.frame( stl( ts( total , frequency = 12 ) , robust = TRUE , s.window = 11 , s.degree = 1 )$time.series ), by = .( tipo_mov , uf , cnae_2_0_secao ) ]
dt_saldo_uf[ , total_d := total - total_season ]


# # additive
# dt_saldo[ , c("total_season" , "total_trend" , "total_remainder") := data.frame( decompose( ts( total , frequency = 12 ) , "additive" )[2:4] ), by = .( tipo_mov ) ]
# dt_saldo[ , c("total_season" , "total_trend" , "total_remainder") := lapply( .SD , as.numeric ), .SDcols = c("total_season" , "total_trend" , "total_remainder") ]
# dt_saldo[ , total_d := total - total_season ]
# 
# dt_saldo_sec <- subset( dt_saldo_sec , !is.na( cnae_2_0_secao ) )
# dt_saldo_sec[ , c("total_season" , "total_trend" , "total_remainder") := data.frame( decompose( ts( total , frequency = 12 ) , "additive" )[2:4] ), by = .( tipo_mov , cnae_2_0_secao ) ]
# dt_saldo_sec[ , c("total_season" , "total_trend" , "total_remainder") := lapply( .SD , as.numeric ), .SDcols = c("total_season" , "total_trend" , "total_remainder") ]
# dt_saldo_sec[ , total_d := total - total_season ]
# 
# dt_saldo_uf <- subset( dt_saldo_uf , !is.na( uf ) )
# dt_saldo_uf <- subset( dt_saldo_uf , !is.na( cnae_2_0_secao ) )
# dt_saldo_uf[ , c("total_season" , "total_trend" , "total_remainder") := data.frame( decompose( ts( total , frequency = 12 ) , "additive" )[2:4] ), by = .( tipo_mov , uf , cnae_2_0_secao ) ]
# dt_saldo_uf[ , c("total_season" , "total_trend" , "total_remainder") := lapply( .SD , as.numeric ), .SDcols = c("total_season" , "total_trend" , "total_remainder") ]
# dt_saldo_uf[ , total_d := total - total_season ]
...
# RESHAPE
these_mvars <- c( "total" , "total_d" , "total_season" , "total_trend" , "total_remainder" )
...
these_labels <- c( "Original" , "Ajustada" , "Sazonalidade" , "Tendência" , "Resto" )
...
p1 <- dt_saldo %>% 
  ...
  .[ variable %in% c( "total" , "total_d" , "total_trend" ) ] %>% 
...
# total
dt_saldo[ , c("mes" , "ano") := list( month( p_date ) , year( p_date ) ) ]
...
ex2 <- dcast( ex2 , ano + mes ~ tipo_mov , value.var = "total_trend" , fill = NA , drop = FALSE )
# ex2[ , Saldo := Admitidos - Desligados ]
...
lapply( these_sectors , function(y) {
  ex <- dcast( dt_saldo_sec[ cnae_2_0_secao == y ] , ano + mes + tipo_mov ~ variable , value.var = "r_value" , fill = NA , drop = FALSE )
  ex <- dcast( ex , ano + mes ~ tipo_mov , value.var = "total_trend" , fill = NA , drop = FALSE )
  ex <- ex[ ano >= 2008 ]
  dcast( ex , mes ~ ano  , value.var = "Saldo" , fill = NA , drop = FALSE )
} )
