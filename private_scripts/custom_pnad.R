...
get_catalog_pnad <-
  function( data_name = "pnad" , output_dir , ... ){
    ...
    catalog$output_filename <- paste0( output_dir , "/" , catalog$year , " main.fst" )
...
lodown_pnad <-
  function( data_name = "pnad" , catalog , ... ){
    ...
    for ( i in seq_len( nrow( catalog ) ) ){
      ...
      fst::write_fst( x , path = catalog[ i , 'output_filename' ] , compress = 100 ) ; rm( x ) ; gc()
...
# define paths
output_dir <- "/home/guilherme/Bases/PNAD"
unlink( output_dir , recursive = TRUE )

# read previously stored catalog
if ( file.exists( file.path( output_dir , "pnad.Rds" ) ) ) { 
  prev_catalog <- readRDS( file.path( output_dir , "pnad.Rds" ) )
} else {
  prev_catalog <- NULL
}

# colelct catalog and build datavault
catalog_init <- get_catalog_pnad( output_dir = output_dir )
catalog <- catalog_init

# build database
built_catalog <- lodown_pnad( catalog = catalog )

# save catalog
built_catalog <- rbind( prev_catalog , built_catalog )
saveRDS( built_catalog , file = file.path( output_dir , "pnad.Rds" ) , compress = TRUE )