...
catalog_caged <-
  function( output_dir , ... ){
    ...
    url_path <- paste0( "ftp://ftp.mtps.gov.br/pdet/microdados/" , c( "CAGED" , "CAGED_AJUSTES" ) , "/" )
    ...
    catalog$type <- ifelse( grepl( "CAGED_AJUSTES" , catalog$full_url ) , "caged_ajustes" , "caged" )
    
    catalog$output_filename <- gsub( "7z$|zip$" , "fst" ,
                                     gsub( paste0( get_common_dir( url_path ) , "/" ) , output_dir , tolower( catalog$full_url ) , ignore.case = TRUE ) ,
                                     ignore.case = TRUE )
    catalog$output_filename <- sapply( catalog$output_filename , utils::URLdecode )
    
    catalog$year <- NULL
    catalog$year[ grepl( "\\.7z$" , basename( catalog$full_url ) ) ] <- gsub( ".*_|\\..*" , "" , grep( "\\.7z$" , basename( catalog$full_url ) , value = TRUE ) ) 
    catalog$year <- ifelse( nchar(catalog$year) >= 6 , substr( catalog$year , 3 ,8 ) , catalog$year )
    catalog$year <- as.numeric( catalog$year )
    
    catalog$month <- NULL
    catalog$month[ grepl( "\\.7z$" , basename( catalog$full_url ) ) ] <- gsub( ".*_|\\..*" , "" , grep( "\\.7z$" , basename( catalog$full_url ) , value = TRUE ) ) 
    catalog$month <- ifelse( nchar(catalog$month) >= 6 , substr( catalog$month , 1 , 2 ) , NA )
    # catalog$month <- as.numeric( catalog$month )
    
    # # each file in a table
    # catalog$db_tablename <-
    #   ifelse( grepl( "\\.7z$" , basename( catalog$full_url ) ) ,
    #           paste0( catalog$type , "_" , catalog$year , ifelse( !is.na( catalog$month ) , catalog$month , "" ) ) , 
    #           NA )
    
    # all files in one table per type
    catalog$db_tablename <- ifelse( grepl( "\\.7z$" , basename( catalog$full_url ) ) , catalog$type , NA )
    ...
    # CAGEDEST_062012.7z tem um erro: ";" sobrando.
...
# build datavault
datavault_caged <- function( catalog , datavault_dir , skipExist = TRUE ) {
...
build_caged <-
  function( catalog , ... ){
    ...
    for ( i in seq_len( nrow( catalog ) ) ){
      ...
      if ( file.exists( catalog[ i , 'output_filename' ] ) ) { next() }
